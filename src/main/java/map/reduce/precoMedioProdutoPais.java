...
public class precoMedioProdutoPais {
    ...
    private static class Pair implements Writable {
        ...
        //Pair vazio = new Pair();
        //Pair leticia = new Pair(new FloatWritable(0f), new FloatWritable(1f));
        //vazio.set(leticia);
        //vazio = leticia;
        //leticia.setPreco(2f);
        ...
        @Override
        public String toString(){ return "{" + preco +"," + nArtigos+ "$}"; }
    }
    ...
    public static class MyMapper extends Mapper<Object, Text, Text, Pair>
    {
        ...
        public void map(Object key, Text value, Context context) throws IOException, InterruptedException
        {
            ...
            if(parts.length == 11){
                try {
                    context.write(new Text(parts[5]+"-"+parts[0]),new Pair(new FloatWritable(Float.parseFloat(parts[8])), new FloatWritable(Float.parseFloat(parts[9]))));
                    // parts[8] - revenue
                    // parts[9] - quantity
                    // (K,V(Pair)) -> (Product-country, revenue(preco),quantity )
                    // average = sum parts[8] (revenue)/ sum parts[9] (quantity)
                }
    ...
    public static void main(String[] args) throws Exception
    {
        ...
        Job job = Job.getInstance(conf, "precoMedioProdutoPais");
        job.setJarByClass(precoMedioProdutoPais.class);
